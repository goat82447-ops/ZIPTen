...
const mongoUri = "mongodb+srv://Akhil:Welcome2@cluster0.yosm6gj.mongodb.net/";
...
const gmailAppPassword = "avcdtwftulbhwclp";
const gmailFromEmail = "routex44@gmail.com";
const twilioAccountSid = process.env.TWILIO_ACCOUNT_SID || "";
const twilioAuthToken = process.env.TWILIO_AUTH_TOKEN || "";
const twilioFromNumber = process.env.TWILIO_FROM_NUMBER || "";
const githubToken = process.env.GITHUB_TOKEN || "";
const githubIssuesRepo =
  process.env.GITHUB_ISSUES_REPO ||
  "goat82447-ops/enterprise-lunchbox-lms-prod";
...
if (twilioAccountSid && twilioAuthToken)
  ...
  gmailTransporter = nodemailer.createTransport({
    service: "gmail",
    auth: { user: gmailFromEmail, pass: gmailAppPassword },
  });
...
const bookingSchema = new mongoose.Schema({
  ...
  preferred_captain_id: String,
  preferred_captain_name: String,
  notification: String,
  ...
  loved_ride: Number,
  loved_captain: Number,
  final_amount: Number,
  payment_done: Number,
  payment_done_at: String,
  tracking_closed: Number,
  tracking_closed_at: String,
  created_at: String,
  updated_at: String,
});
...
const CaptainFeedback = mongoose.model(
  "CaptainFeedback",
  captainFeedbackSchema,
);
...
function heuristicRcaAnalysis(issue) {
  ...
  if (
    ...
    signals.push("feature-regression");
  if (
    text.includes("login") ||
    text.includes("auth") ||
    text.includes("otp") ||
    ...
    signals.push("auth-flow");
  if (text.includes("500") || text.includes("internal server error"))
    signals.push("backend-exception");
  if (
    text.includes("cors") ||
    text.includes("network") ||
    text.includes("fetch failed")
  )
    signals.push("network-cors");
  if (
    text.includes("save") ||
    text.includes("localstorage") ||
    text.includes("refresh")
  )
    signals.push("state-persistence");
  if (text.includes("map") || text.includes("location") || text.includes("gps"))
    signals.push("geo-permission");
      ...
      messages: [
        {
          role: "system",
          content:
            "You produce practical production bug RCA comments for GitHub issues.",
        },
...
async function postGithubIssueComment(repoFullName, issueNumber, body) {
  ...
  if (!response.ok) {
    throw new Error(
      payload?.message ||
        `GitHub comment creation failed with status ${response.status}`,
    );
  }
...
async function generateRcaComment(issue) {
  ...
  try {
    ...
    console.warn(
      "[Bug-RCA-Agent] AI analysis failed, using heuristic fallback:",
      err.message,
    );
  }
  ...
  const signalText = heuristic.signals.length
    ? heuristic.signals.join(", ")
...
async function createGithubIssueForBugReport({
  type,
  subject,
  name,
  contact,
  ...
  const response = await fetch(
    `https://api.github.com/repos/${githubIssuesRepo}/issues`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${githubToken}`,
...
function mapBookingRow(row) {
  return {
    ...
    currentLocation: safeJson(row.current_location_json, {
      lat: 0,
      lng: 0,
      address: "",
    }),
    ...
    estimatedFare:
      row.estimated_fare != null ? Number(row.estimated_fare) : undefined,
    ...
    captainRating:
      row.captain_rating != null ? Number(row.captain_rating) : undefined,
    lovedRide: toBool(row.loved_ride),
    lovedCaptain: toBool(row.loved_captain),
    finalAmount:
      row.final_amount != null ? Number(row.final_amount) : undefined,
...
function isCaptainAssignedToBooking(session, row) {
  const target = String(row.notification_target || "all")
    .trim()
    .toLowerCase();
  ...
  return (
    ...
    (row.preferred_captain_id &&
      String(row.preferred_captain_id).toLowerCase() === cId) ||
    (row.driver_name && String(row.driver_name).toLowerCase() === cUser) ||
    (row.preferred_captain_name &&
      String(row.preferred_captain_name).toLowerCase() === cName)
  );
}
...
async function sendEmailOtp(email, otp) {
  console.log("gmailTransporter: ",gmailTransporter);
  console.log("gmailFromEmail: ",gmailFromEmail);
  if (gmailTransporter && gmailFromEmail) {
    await gmailTransporter.sendMail({
      from: gmailFromEmail,
      to: email,
      subject: "Your RouteX OTP",
...
app.get("/", (_req, res) =>
  res.json({ service: "routex-backend", status: "ok", version: "1.0.0" }),
);
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    if (normalizedRole === "captain" && !captainVehicle)
      return res
        .status(400)
        .json({ error: "captainVehicle is required for captains." });
    ...
    try {
      await sendEmailOtp(email.trim().toLowerCase(), emailOtp);
    } catch (e) {
    ...
    const payload = {
      ...
      requiresOtp: true,
      tempToken,
      channels: { email: email.trim().toLowerCase() },
    };
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (
      requestedRole &&
      requestedRole !== String(user.role || "").toLowerCase()
    )
      return res
        .status(401)
        .json({
          error: "Selected login mode does not match your account role.",
        });
    if (user.role === "customer" && Number(user.customer_otp_completed) !== 1)
      return res
        .status(403)
        .json({ error: "Complete OTP verification first." });
...
app.post("/api/auth/verify-otp", async (req, res) => {
  try {
    ...
    if (!code || code.code !== String(emailOtp || "").trim())
...
app.post("/api/auth/profile-image", requireSession, async (req, res) => {
  ...
  await User.updateOne(
    ...
    { $set: { profile_image: String(profileImageUrl).trim() } },
  );
  return res.json({
...
app.delete("/api/auth/account", requireSession, async (req, res) => {
  await Promise.all([
    ...
    AuthSession.deleteMany({ user_id: req.session.user_id }),
    OtpCode.deleteMany({ user_id: req.session.user_id }),
  ]);
...
app.get("/api/auth/actions", requireSession, async (req, res) => {
  ...
  const rows = await UserAction.find(query)
    .sort({ created_at: -1 })
    .limit(100)
    .lean();
  return res.json(
...
app.get("/api/auth/captains", requireSession, async (req, res) => {
  ...
  const query = vehicleType
    ? { role: "captain", captain_vehicle: String(vehicleType) }
    : { role: "captain" };
  const captains = await User.find(query)
    .select("_id username display_name mobile captain_vehicle profile_image")
    .lean();
  return res.json(
    captains.map((c) => ({
      id: c._id,
      ...
      phone: c.mobile,
      vehicleType: c.captain_vehicle || undefined,
...
app.get("/api/auth/users", requireSession, async (req, res) => {
  ...
  const users = await User.find({})
    .select(
      "_id username display_name email mobile role captain_vehicle created_at",
    )
    .sort({ created_at: -1 })
    .lean();
  return res.json(
...
app.post("/api/auth/captain-feedback", requireSession, async (req, res) => {
  ...
  if (!bookingId || !captainName || !rideRating || !captainRating)
    return res
      ...
      .json({
        error:
          "bookingId, captainName, rideRating, captainRating are required.",
      });
  const now = nowIso();
  const existing = await CaptainFeedback.findOne({
    booking_id: String(bookingId),
  });
  const data = {
    captain_user_id: captainId || null,
    captain_name: String(captainName).trim(),
    submitted_by_user_id: req.session.user_id,
    submitted_by_name: req.session.display_name || req.session.username,
    ride_rating: Number(rideRating),
    captain_rating: Number(captainRating),
    feedback_text: feedbackText || null,
    loved_ride: lovedRide ? 1 : 0,
    loved_captain: lovedCaptain ? 1 : 0,
    updated_at: now,
  };
  if (existing) {
    await CaptainFeedback.updateOne(
      { booking_id: String(bookingId) },
      { $set: data },
    );
  } else {
...
app.get(
  "/api/auth/captain-feedback/stats",
  requireSession,
  async (req, res) => {
    if (!["captain", "admin"].includes(req.session.role))
      return res.status(403).json({ error: "Captain or admin role required." });
    const captainUserId =
      req.session.role === "admin"
        ? req.query.captainId || ""
        : req.session.user_id;
    const allFeedback = await CaptainFeedback.find({
      captain_user_id: captainUserId,
    }).lean();
    const feedbackCount = allFeedback.length;
    const avgCaptainRating = feedbackCount
      ? Number(
          (
            allFeedback.reduce((s, f) => s + Number(f.captain_rating || 0), 0) /
            feedbackCount
          ).toFixed(1),
        )
      : 0;
    const avgRideRating = feedbackCount
      ? Number(
          (
            allFeedback.reduce((s, f) => s + Number(f.ride_rating || 0), 0) /
            feedbackCount
          ).toFixed(1),
        )
      : 0;
    const totalHearts = allFeedback.reduce(
      (s, f) => s + Number(f.loved_captain || 0) + Number(f.loved_ride || 0),
      0,
    );
    const recentComments = await CaptainFeedback.find({
      captain_user_id: captainUserId,
      feedback_text: { $nin: [null, ""] },
    })
      .sort({ created_at: -1 })
      .limit(8)
      .lean();
    return res.json({
      ...
      recentComments: recentComments.map((r) => ({
        ...
        userName: r.submitted_by_name,
        rideRating: r.ride_rating,
        captainRating: r.captain_rating,
        feedbackText: r.feedback_text,
        lovedRide: toBool(r.loved_ride),
        lovedCaptain: toBool(r.loved_captain),
        createdAt: r.created_at,
      })),
    });
  },
...
function notifyRideAudience(eventName, data, customerUserId) {
  ...
  for (const [id, client] of sseClients) {
    ...
    const isBookingCustomer =
      customerId.length > 0 && String(client.userId || "") === customerId;
...
app.post("/api/bookings", requireSession, async (req, res) => {
  try {
    ...
    if (!pickup.address || !drop.address)
      return res
        .status(400)
        .json({ error: "pickup and drop addresses are required." });
    const otp = genOtp();
    const bookingId = String(
      body.clientRequestId || `BK-${Date.now().toString().slice(-8)}`,
    ).trim();
    if (!bookingId) {
    ...
    await Booking.create({
      ...
      current_location_json: JSON.stringify({
        address: String(pickup.address),
        lat: Number(pickup.lat || 0),
        lng: Number(pickup.lng || 0),
      }),
      ...
      notification:
        notificationTarget === "all"
          ? `Booking confirmed. OTP ${otp}. Broadcast to all captains.`
          : `Booking confirmed. OTP ${otp}. Preferred captain notified.`,
      estimated_fare:
        body.estimatedFare != null ? Number(body.estimatedFare) : null,
  ...
  async (req, res) => {
    ...
    await Booking.updateOne(
      { _id: booking._id },
      {
        $set: {
          status: "assigned",
          notification: "Captain accepted the ride and is on the way.",
          updated_at: nowIso(),
        },
      },
    );
    ...
    await Booking.updateOne(
      { _id: booking._id },
      {
        $set: {
          otp_verified: 1,
          status: "assigned",
          notification: "OTP verified. Ride started.",
          updated_at: nowIso(),
        },
      },
    );
  ...
  async (req, res) => {
    ...
    await Booking.updateOne(
      { _id: booking._id },
      {
        $set: {
          status: "cancelled",
          notification: `Ride cancelled by ${req.body.role || "user"}.`,
          updated_at: nowIso(),
        },
      },
    );
...
app.post("/api/bookings/:bookingId/sos", requireSession, async (req, res) => {
  ...
  await Booking.updateOne(
    { _id: booking._id },
    {
      $set: {
        sos_triggered: 1,
        sos_by_role: req.body.role || "customer",
        notification: "SOS triggered. Emergency help alerted.",
        updated_at: nowIso(),
      },
    },
  );
...
app.post(
  ...
  async (req, res) => {
    const booking = await Booking.findById(req.params.bookingId).lean();
    if (!booking) return res.status(404).json({ error: "Booking not found." });
    const { rideRating, captainRating, feedbackText, lovedRide, lovedCaptain } =
      req.body || {};
    await Booking.updateOne(
      { _id: booking._id },
      {
        $set: {
          ...
          ride_rating: Number(rideRating || 0),
          captain_rating: Number(captainRating || 0),
          feedback_text: feedbackText || null,
          loved_ride: lovedRide ? 1 : 0,
          loved_captain: lovedCaptain ? 1 : 0,
          notification: "Feedback submitted. Thank you!",
          updated_at: nowIso(),
        },
      },
    );
  ...
  await Booking.updateOne(
    { _id: booking._id },
    {
      $set: {
        ...
        payment_done: 1,
        payment_done_at: nowIso(),
        notification: `Payment of Rs ${finalAmount} completed.`,
        updated_at: nowIso(),
      },
    },
  );
...
app.post(
  "/api/bookings/:bookingId/close-tracking",
  requireSession,
  async (req, res) => {
    const booking = await Booking.findById(req.params.bookingId).lean();
    if (!booking) return res.status(404).json({ error: "Booking not found." });
    await Booking.updateOne(
      { _id: booking._id },
      {
        $set: {
          tracking_closed: 1,
          tracking_closed_at: nowIso(),
          status: "completed",
          notification: "Tracking closed. Trip completed.",
          updated_at: nowIso(),
        },
      },
    );
...
app.get("/api/integrations/health", async (_req, res) => {
  ...
  return res.json({
    ...
    checkedAt: nowIso(),
    integrations: [
      { name: "MongoDB", status: mongoOk ? "live" : "down" },
...
app.get("/api/menu/hotels/:hotelId/items", (req, res) => {
  ...
  return res.json({
    ...
    updatedAt: nowIso(),
    items: menus[req.params.hotelId] || menus.h1,
  });
});
...
app.post("/api/pricing/live-fare", async (req, res) => {
  ...
  return res.json({
    distanceKm: 5,
    durationInTrafficMinutes: 20,
    trafficCondition: "medium",
    weatherCondition: "clear",
    breakdown: { baseFare: 55, distanceFare: rate * 5, total: 55 + rate * 5 },
  });
});

app.post("/api/promos/validate", (req, res) => {
  const catalog = [
    { code: "SAVE10", type: "percent", value: 10, minAmount: 120 },
    { code: "FLAT50", type: "flat", value: 50, minAmount: 250 },
    { code: "FIRST100", type: "flat", value: 100, minAmount: 500 },
  ];
  const promo = catalog.find(
    (p) => p.code === String(req.body.code || "").toUpperCase(),
  );
  if (!promo)
  ...
  if (amount < promo.minAmount)
    return res
      .status(400)
      .json({ valid: false, error: `Minimum Rs ${promo.minAmount} required.` });
  const discount =
    promo.type === "flat"
      ? promo.value
  ...
  return res.json({
    ...
    code: promo.code,
    discount,
    payableAmount: Math.max(0, amount - discount),
  });
});
...
app.post("/api/support/complaints", async (req, res) => {
  ...
  if (!type || !subject || !description)
    return res
      .status(400)
      .json({ error: "type, subject, description required." });
  ...
  try {
    const githubIssue = await createGithubIssueForBugReport({
      type,
      subject,
      name,
      contact,
    ...
    console.log(
      `[Support] type=${type} subject=${subject}${githubIssue ? ` issue=#${githubIssue.issueNumber}` : ""}`,
    );
    ...
    return res
      .status(502)
      .json({ error: `Failed to raise bug in GitHub: ${error.message}` });
  }
});
...
app.post("/api/support/app-feedback", async (req, res) => {
  ...
  return res
    .status(201)
    .json({ message: "App feedback submitted successfully." });
});
...
app.post("/api/github/webhook", async (req, res) => {
  try {
    ...
    if (event !== "issues") {
      return res
        .status(200)
        .json({ message: "Ignored: unsupported event type." });
    }
    ...
    if (!issue || !repository) {
      return res
        .status(200)
        .json({ message: "Ignored: missing issue or repository payload." });
    }
    ...
    if (issue.pull_request) {
      return res
        .status(200)
        .json({ message: "Ignored: pull request issue payload." });
    }

    const labels = Array.isArray(issue.labels)
      ? issue.labels.map((l) => String(l?.name || "").toLowerCase())
      : [];
    const isBug =
      labels.includes("bug") ||
      String(issue.title || "")
    ...
    if (!shouldAnalyze) {
      return res
        .status(200)
        .json({ message: "Ignored: not an actionable bug issue event." });
    }

    const comment = await generateRcaComment(issue);
    await postGithubIssueComment(repository.full_name, issue.number, comment);
    console.log(
      `[Bug-RCA-Agent] Commented on ${repository.full_name}#${issue.number}`,
    );
    ...
    return res
      .status(500)
      .json({ error: `Webhook processing failed: ${error.message}` });
  }
});
